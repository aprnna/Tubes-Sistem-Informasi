      ...
      <div className="flex flex-col space-y-4 relative">
        {filteredMenuItems.map((item, index) => (
          ...
          >
            ...
            {!isExpanded && (
              <div className={`absolute left-full top-1/2 transform -translate-y-1/2 bg-amber-900 text-white shadow-md p-2 px-4 rounded ml-2 transition-all duration-300 ${hoveredItem === index ? 'opacity-100 z-50' : 'opacity-0 -z-50'}`}>
                {item.text}
              </div>
            )}
          </Link>
