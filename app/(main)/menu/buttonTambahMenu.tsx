...
export default function ButtonTambah(){
    ...
    const getCategories = async () => {
        const { data }: { data: { kategori: string }[] } = await fetchApi("/menu", "GET");
    ...
    return(
        <>
            <button 
                className="text-lg font-normal bg-amber-900 hover:bg-amber-950 text-slate-50 px-8 rounded-lg transition-all duration-300"
                onClick={() => handleTambahMenu()}
            >
                Tambah Menu
            </button>
            {showMenuModal && (
                <Modal key={showMenuModal ? 'show' : 'hide'} closeModal={() => setShowMenuModal(false)} showModal={showMenuModal}>
                <div className="min-w-[756px]">
                    ...
                    <div className="flex gap-4">
                        <div className="flex flex-col w-1/2 text-base font-normal gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                                <div className="flex justify-between">
                                    <label className="font-medium" htmlFor="hargaJual">Kategori</label>
                                    <span className="text-slate-500">Required</span>
                                </div>
                                <div className="flex border-slate-200 border rounded-lg">
                                    <input
                                        disabled 
                            ...
                            <div className="flex-col p-2 border-slate-200 border rounded-lg max-h-40 overflow-auto">
                                {isAdding ? (
                                    ...
                                    <button className="flex gap-3 hover:bg-red-50 w-full py-1 rounded-lg" onClick={handleAddClick}>
                                        <svg width="26" height="25" viewBox="0 0 26 25" xmlns="http://www.w3.org/2000/svg" className="text-slate-900">
                                ...
                                <div>
                                    {categories.map((category, index) => (
                                        <button key={index} className="mt-2 flex gap-3 hover:bg-red-50 w-full py-1 rounded-lg" onClick={() => handleCategoryClick(category)}>
