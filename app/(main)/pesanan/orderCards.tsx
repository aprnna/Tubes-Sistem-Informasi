            ...
            {loading ? (
                ...
                <>
                    {orders.map((order: any, index) => (
                        <div key={index} className="flex flex-col items-center justify-center min-w-72 rounded-2xl bg-white shadow-lg p-4">
            ...
            <div className=" mt-12 mr-12 items-center min-w-[512px] justify-center rounded-t-lg py-4 px-6">
                ...
                orderData && (
                    ...
                    <div className="flex flex-col gap-1 pt-1.5 text-sm text-gray-600">
                        ...
                        <h4>ID Pesanan</h4>
                        <input disabled className="text-end bg-white font-medium" type="text" value={formatID(orderData.order[0].id)} />
                        </div>
                        <div className="flex justify-between">
                        <h4>Atas Nama</h4>
                        <input disabled className="text-end bg-white font-medium" type="text" value={orderData.order[0].atasNama} />
                        </div>
                        <div className="flex justify-between">
                        <h4>Jumlah Orang</h4>
                        <input disabled className="text-end bg-white font-medium" type="number" value={orderData.order[0].banyak_orang} />
                        </div>
                        <div className="flex justify-between">
                        <h4>Tanggal Pesanan</h4>
                        <input disabled className="text-end bg-white font-medium " type="timestamp" value={formatToDateTimeLocal(orderData.order[0].created_at)} />
                        </div>
                        <div className="flex justify-between">
                        <h4 className="w-1/2">ID Kasir</h4>
                        <input disabled className="text-end bg-white font-medium w-full" type="text" value={orderData.order[0].id_users} />
                        </div>
                    </div>
                    <div className="flex flex-col mt-2 min-h-48 max-h-48">
                        ...
                        {orderData.menuDetails.map((item) => (
                            ...
                            <div className="flex flex-row items-center text-sm font-semibold">
                                <span>Qty: {orderData.items.map((qty) =>(
